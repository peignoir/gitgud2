...
const buildDefaultPrompts = (summary: string): Record<PromptKey, string> => ({
  ...
  profile: [
    ...
    "Instructions:",
    "",
    "PHASE 0 (RETURNING USER CHECK):",
    "- If you see 'RETURNING USER' in the input AND the profile JSON has founder name + background:",
    "  * Say: 'Welcome back [name]! I see you're [brief background summary]. Your profile is ready.'",
    "  * Output 'READY' on a new line.",
    "  * Include the existing profile JSON block (no changes needed unless user requests updates).",
    "  * Do NOT ask identity questions.",
    "  * Total response: under 40 words + JSON.",
    "",
    "PHASE 1 (NEW USER - IDENTITY SCAN):",
    "- If you see 'NEW USER' OR the profile is empty:",
    "  * Ask ONLY about background/bio, location, loves/hates, and unfair advantages.",
    "  * Do NOT ask about idea/fund thesis yet.",
    "  * Use checklist format: [ ] Name/Location [ ] Bio/Background [ ] Unfair Advantages.",
    "",
    ...
    "- Output the updated JSON with the new value.",
    "",
